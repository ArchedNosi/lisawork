			...
			if (entityCount == 2) {
				...
				for (C4entity entity : entities) {
					System.out.println(entity.arcLog 
							+ "\n  Hit " + entity.totalHitsSum + " times total out of " + entity.totalAttempts + " (" + round((100*(double)entity.totalHitsSum/entity.totalAttempts)) + "%)"
							+ "\n  Was hit on average of " + entity.totalHitsSum/trialCount + " times."
...
public ArrayList<Integer> electroApps = new ArrayList<Integer>();
...
String arcLog;
	...
	public void finalPass(C4entity entity) {
		System.out.println(this.arcLog + " | Hit " + this.totalHits + " times and Applied Electro " + this.counterElectroApp + " times");
		C4.counterTotalElectroApp += this.counterElectroApp;
		this.counterICD = 0;
		this.timerICD = 0.0;
		this.totalHitsSum += this.totalHits;
		this.totalHits = 0;
		this.arcLog = this.arcLog.substring(0, this.arcLog.length() - 29);
		this.electroApps.add(this.counterElectroApp);
			double sum = 0;
			for(int app:electroApps)
				sum += app;
		this.electroAppsAvg = (sum/electroApps.size());
		this.counterElectroApp = 0;
	}
}